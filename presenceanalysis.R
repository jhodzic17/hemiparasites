...
data.eco <- read.csv("PARKHEMIFINAL/parkhemi.eco.csv", header = TRUE)
area <- read.csv("plots_areasampled.csv", header = TRUE); head(area)
...
master.JH <- master.JH %>% filter(L4.same == "Y" & park.same == "Y") ## same egorecion and same park
...
## can use this to calculate differences between the two
...
##  can also pivot longer and use GLM -- used in paper so area could be random effect
...
head(master.JH)
colnames(master.JH)
master.JH <- master.JH %>% select(unique.pair, ALLRICH.IN:NOHEMIAB.IN, ALLRICH.NEAR:NOHEMIAB.NEAR , Area_sampled.IN, Area_sampled.NEAR)

head(master.JH)

master.JH <- master.JH%>% filter(!is.na(Area_sampled.IN), !is.na(Area_sampled.NEAR))
...
alleven.mod <- lmer(ALLEVEN.logit ~ Area_sampled + HEMI.status + (1|unique.pair), master.long ); summary(alleven.mod)
alleven.mod.n <- lmer(ALLEVEN.logit ~ Area_sampled + (1|unique.pair), master.long ) ## without hemiparasite

deltaAIC = AIC(alleven.mod.n) - AIC(alleven.mod); deltaAIC ## with hemiparasite is significant
...
herbeven.mod <- lmer(HERBEVEN.logit ~ Area_sampled + HEMI.status + (1|unique.pair), master.long); summary(herbeven.mod)
herbeven.mod.n <- lmer(HERBEVEN.logit ~ Area_sampled + (1|unique.pair), master.long )
deltaAIC = AIC(herbeven.mod.n) - AIC(herbeven.mod); deltaAIC ## hemip not significant for herb

treeeven.mod <- lmer(TREEEVEN.logit ~ Area_sampled + HEMI.status + (1|unique.pair), master.long ); summary(treeeven.mod)
treeeven.mod.n <- lmer(TREEEVEN.logit  ~ Area_sampled  + (1|unique.pair), master.long )
deltaAIC = AIC(treeeven.mod.n) - AIC(treeeven.mod); deltaAIC ## significant
tab_model(treeeven.mod)

## RICHNESS

allrich.mod <- lmer(ALLRICH.log ~ Area_sampled + HEMI.status + (1|unique.pair), master.long); summary(allrich.mod)
allrich.mod.n <- lmer(ALLRICH.log ~ Area_sampled + (1|unique.pair), master.long); summary(allrich.mod.n)
deltaAIC = AIC(allrich.mod.n ) - AIC(allrich.mod); deltaAIC ## significant


herbrich.mod <- lmer(HERBRICH.log ~ Area_sampled + HEMI.status + (1|unique.pair), master.long); summary(herbrich.mod)
herbrich.mod.n <- lmer(HERBRICH.log ~ Area_sampled + (1|unique.pair), master.long); summary(herbrich.mod.n)
deltaAIC = AIC(herbrich.mod.n) - AIC(herbrich.mod); deltaAIC



treerich.mod <- lmer(TREERICH.log ~ Area_sampled + HEMI.status + (1|unique.pair), master.long); summary(treerich.mod)
treerich.mod.n <- lmer(TREERICH.log ~ Area_sampled + (1|unique.pair), master.long); summary(treerich.mod.n)
